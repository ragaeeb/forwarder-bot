...
describe('requireNewSetup', () => {
    ...
    it('should pass if setup was never completed', async () => {
        await requireNewSetup({ botUsername: 'testbot' } as unknown as ForwardContext, next);
    ...
    it('should pass if setup was completed with a different group', async () => {
        await requireNewSetup({ botUsername: 'testbot', chat: { id: 2 }, settings: { adminGroupId: '1' } } as unknown as ForwardContext, next);
    ...
    it('should not pass if setup was completed for the same group', async () => {
        await requireNewSetup({ botUsername: 'testbot', chat: { id: 1 }, settings: { adminGroupId: '1' } } as unknown as ForwardContext, next);
